					...
					int ncmdshow)
{
    ...
    winclass.hIcon			= LoadIcon(nullptr, MAKEINTRESOURCE(IDI_T3DX));
    ...
    std::vector<ALLMONITORINFO> monitorInfo;

    monitorInfo.reserve(GetSystemMetrics(SM_CMONITORS));
    ...
    {
        if (!monitorInfo[i].isPrimary)
        {
            RECT rect = monitorInfo[i].rect;
    ...
    {    
        ...
        for (int col = 0; col < NUM_BLOCK_COLUMNS; col++)
        {
            ...
            if (blocks[row][col] != 0)
            {
                ...
                PALETTEENTRY paletteEntry = palette[blocks[row][col]];

                Draw_Rectangle(x1, y1, x1 + BLOCK_WIDTH,
                        y1 + BLOCK_HEIGHT, RGB32(paletteEntry.peRed,
...
void Process_Ball(void)
{
    ...
    if (ball_y > (SCREEN_HEIGHT / 2) && ball_dy > 0)
    {
           ...
           (y >= paddle_y && y <= paddle_y + PADDLE_HEIGHT))
       {
           ...
           // make a little noise
           //MessageBeep(MB_OK);
           PlaySound(L"hit_paddle.wav", nullptr, SND_ASYNC);
	...
	// now scan through all the blocks and see of ball hit blocks
	for (int row = 0; row < NUM_BLOCK_ROWS; row++)
	{
		// reset column position
		x1 = BLOCK_ORIGIN_X;
		...
		for (int col = 0; col < NUM_BLOCK_COLUMNS; col++)
		{
			...
			if (blocks[row][col] != 0)
			{
					...
					(ball_cy > y1) && (ball_cy < y1 + BLOCK_HEIGHT))
				{
                    ...
                    PlaySound(L"hit_block.wav", nullptr, SND_ASYNC);
...
int Game_Main(void *parms)
{
    ...
    wchar_t buffer[80]; // used to print text

    // what state is the game in? 
    if (game_state == GAME_STATE_INIT)
    {
        // initialize everything here graphics
        if (DD_Init(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_BPP) != DD_OK)
        {
            return 0;
        }
		...
		// 打开音频文件
        ::wsprintf(szCmd, L"open \"%s\" alias Catalyst", L"Catalyst.wav");
		::mciSendString(szCmd, nullptr, 0, nullptr);
		// 播放音频文件
		::mciSendString(L"play Catalyst", NULL, 0, NULL);
		// 关闭文件, 停止播放
		//::mciSendString(L"close Catalyst", NULL, 0, NULL);
    ...
    else if (game_state == GAME_STATE_RUN)
    {
        ...
        Draw_Rectangle(0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, RGB32(100, 149, 237));
        ...
        else if (ball_y > (SCREEN_HEIGHT - BALL_SIZE))
        {
            ...
            PlaySound(L"fail.wav", nullptr, SND_ASYNC);
        ...
        Draw_Rectangle(paddle_x - 2, paddle_y + 2, 
                       paddle_x+PADDLE_WIDTH - 2, 
                       paddle_y+PADDLE_HEIGHT + 2, 0);

        Draw_Rectangle(paddle_x, paddle_y, 
                       paddle_x + PADDLE_WIDTH, 
                       paddle_y + PADDLE_HEIGHT, RGB32(191, 191, 191));

        // draw the ball
        Draw_Rectangle(ball_x - 2, ball_y + 2, ball_x + BALL_SIZE - 2, 
                       ball_y + BALL_SIZE + 2, 0);
        Draw_Rectangle(ball_x, ball_y, ball_x + BALL_SIZE, 
                       ball_y + BALL_SIZE, RGB32(255, 255, 255));

        // draw the info
        wsprintf(buffer, L"F R E A K O U T   Score %d    Level %d", score, level);
        Draw_Text_GDI(buffer, 16, SCREEN_HEIGHT - 32, RGB32(255, 255, 255));
